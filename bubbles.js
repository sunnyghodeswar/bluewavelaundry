(function () {
  ...
  function launch(b, aloft, src) {
    src = src || pickDrum();
    var tiny = src.fromNav || src.fromCard || Math.random() < 0.5;
    b.r = src.fromCard ? rand(8, 18) : src.fromNav ? rand(7, 16) : (tiny ? rand(10, 18) : rand(18, 36));
    b.x0 = src.x;
    b.y0 = src.y;
    b.dir = src.fromNav ? 1 : -1;
  ...
  function tick(now) {
    ...
    for (var i = 0; i < bubbles.length; i++) {
      ...
      if ((b.dir < 0 && y < -40) || (b.dir > 0 && y > pageH + 40)) {
        if (b.cardId) {
          dropBubble(i);
          i--;
        } else {
  ...
  var wait = setInterval(function () {
    ...
    if (found.length) {
      drums = found;
      for (var i = 0; i < bubbles.length; i++) {
        if (!bubbles[i].cardId) launch(bubbles[i], true);
      }
      sizeField();
      clearInterval(wait);
    } else if (tries > 50) {
